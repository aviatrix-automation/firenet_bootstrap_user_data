  ...
  peerings = flatten([
    for gw1 in local.transit_gateways : [
      ...
      for gw2 in slice(local.transit_gateways, index(local.transit_gateways, gw1) + 1, length(local.transit_gateways)) : {
        ...
        excluded_cidrs1 = try(
          flatten([for j in var.custom_excluded_cidrs : j["${gw1}~~${gw2}"] if contains(keys(j), "${gw1}~~${gw2}")]),
          flatten([for j in var.custom_excluded_cidrs : j["${gw2}~~${gw1}"] if contains(keys(j), "${gw2}~~${gw1}")]),
          null
        )
        excluded_cidrs2 = try(
          flatten([for j in var.custom_excluded_cidrs : j["${gw1}~~${gw2}"] if contains(keys(j), "${gw1}~~${gw2}")]),
...
resource "aviatrix_transit_gateway_peering" "peering" {
  ...
  gateway1_excluded_network_cidrs             = length(each.value.excluded_cidrs1) == 0 ? var.excluded_cidrs : each.value.excluded_cidrs1
  gateway2_excluded_network_cidrs             = length(each.value.excluded_cidrs2) == 0 ? var.excluded_cidrs : each.value.excluded_cidrs2
