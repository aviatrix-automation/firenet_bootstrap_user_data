...
variable "custom_excluded_cidrs" {
  description = "A per-peering specific list of CIDR's to exclude on each peering. This variable takes precedence over the global variable excluded_cidrs."
  type    = list(map(list(string)))
  default = null

  validation {
    condition = alltrue([for id in var.custom_excluded_cidrs : length(regexall("\\w~{2}\\w", one(keys(id)))) > 0 ? true : false])
    error_message = "Please enter each Transit peering ID using the following format: {gw_name}~~{gw_name}"
  }
}
